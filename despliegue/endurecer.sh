...
cat > /etc/apt/apt.conf.d/51myprogressplan <<'CONF'
...
cat > /etc/ssh/sshd_config.d/60-myprogressplan.conf <<'CONF'
...
cat > /etc/fail2ban/jail.d/myprogressplan.conf <<'CONF'
[sshd]
enabled  = true
maxretry = 4
bantime  = 1h
...
if [[ -f /etc/myprogressplan/config.env ]]; then
    chown root:myprogressplan /etc/myprogressplan/config.env 2>/dev/null || chown root:root /etc/myprogressplan/config.env
    chmod 640 /etc/myprogressplan/config.env
    ls -l /etc/myprogressplan/config.env
fi

paso "MySQL solo en local"
if [[ -d /etc/mysql/mysql.conf.d ]]; then
    cat > /etc/mysql/mysql.conf.d/99-myprogressplan.cnf <<'CONF'
[mysqld]
...
echo "  volumen:    $(mountpoint -q /var/lib/myprogressplan && echo montado || echo 'NO montado')"

cat <<'PENDIENTE'

Lo que este script NO hace y hay que hacer a mano:

  1. Certificado: sudo certbot --nginx -d myprogressplan.com -d www.myprogressplan.com
